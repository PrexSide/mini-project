...
//get transactions
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      const payload = {
        ...values,
        date: values.date.format("YYYY-MM-DD"), // or ISO string if your backend prefers it
        userId: user._id,
      };
      ...
      if (editable) {
        ...
        await axios.post("/transaction/add-transaction", payload);
      ...
      setLoading(false);
      console.error("Add/Edit error:", error); // helpful for debugging
      message.error(editable ? "Failed to update transaction" : "Failed to add transaction");
    }
  };
